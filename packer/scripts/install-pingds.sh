...
echo 
echo "========================================"
echo "Installing PingDS ${PINGDS_VERSION} at $(date)"
...
if [ ! -d /opt/DS-${PINGDS_VERSION} ]; then
   echo "Downloading DS-${PINGDS_VERSION}.zip at $(date)"
   ...
   do               
      if [ "${CLOUD_PLATFORM}" == "aws" ]; then
         get_stored_file "${aws_storage_account}" "DS-${PINGDS_VERSION}.zip" "${AIL_INSTALL}/"
         retVal=$?
      fi

      if [ "${CLOUD_PLATFORM}" == "azure" ]; then

         get_stored_file "${BLOB_CONTAINER_NAME}" "DS-${PINGDS_VERSION}.zip" \
                         "${AIL_INSTALL}/DS-${PINGDS_VERSION}.zip" \
                         "${BLOB_STORAGE_ACCOUNT_NAME}" "${BLOB_RESOURCE_GROUP_NAME}"
         retVal=$?
      fi

      if [ ${retVal} -ne 0 ]; then
         echo " - Waiting for ${CLOUD_PLATFORM} storage \"${aws_storage_account}/DS-${PINGDS_VERSION}.zip\" to return with non-zero status, currently ${retVal}"
         sleep 5
      fi
   done

   unzip -q "/opt/AIL/install/DS-${PINGDS_VERSION}.zip" \
    -x *.exe *.dll *.bat  



#   rm -f "/opt/AIL/install/DS-${PINGDS_VERSION}.zip"
else
   echo "$0: Not downloading DS-${PINGDS_VERSION}, already installed"
fi
