...
echo 
echo "========================================"
echo "Installing PingIDM ${PINGIDM_VERSION} at $(date)"
...
if [ ! -d /opt/IDM-${PINGIDM_VERSION} ]; then
   echo "Downloading IDM-${PINGIDM_VERSION}.zip at $(date)"
   ...
   do               
      if [ "${CLOUD_PLATFORM}" == "aws" ]; then
         get_stored_file "${aws_storage_account}" "IDM-${PINGIDM_VERSION}.zip" "${AIL_INSTALL}/"
         retVal=$?
      fi

      if [ "${CLOUD_PLATFORM}" == "azure" ]; then

         get_stored_file "${BLOB_CONTAINER_NAME}" "IDM-${PINGIDM_VERSION}.zip" \
                         "${AIL_INSTALL}/IDM-${PINGIDM_VERSION}.zip" \
                         "${BLOB_STORAGE_ACCOUNT_NAME}" "${BLOB_RESOURCE_GROUP_NAME}"
         retVal=$?
      fi

      if [ ${retVal} -ne 0 ]; then
         echo " - Waiting for ${CLOUD_PLATFORM} storage \"${aws_storage_account}/IDM-${PINGIDM_VERSION}.zip\" to return with non-zero status, currently ${retVal}"
         sleep 5
      fi
   done

   unzip -q "/opt/AIL/install/IDM-${PINGIDM_VERSION}.zip" \
    -x *.exe *.dll *.bat  



#   rm -f "/opt/AIL/install/IDM-${PINGIDM_VERSION}.zip"
else
   echo "$0: Not downloading IDM-${PINGIDM_VERSION}, already installed"
fi
...
echo "Installed PingIDM at $(date)"
echo "========================================"
echo ""
echo ""
