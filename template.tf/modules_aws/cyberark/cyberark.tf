resource "aws_instance" "cyberark_server" {
  ...
  ami           = data.aws_ami.cyberark_ami.id
...
EOF
  ...
  vpc_security_group_ids = concat([var.security_group_ids["${var.product_name}"]], aws_security_group.cyberark[*].id)


  tags = {
    Name = "Windows-Cyberark-VM"
  }
}

//output "admin_passwords" {
//  value = [
//    for instance in aws_instance.cyberark_server : 
