resource "aws_instance" "pingidm_server" {
  ...
  ami = data.aws_ami.pingidm_ami.id

  user_data = templatefile("${path.module}/scripts/pingidm.sh", {
        BUILD_VERSION                          = data.aws_ami.pingidm_ami.name
        ...
        LOG_RETENTION                          = try(var.products[var.product_name].parameters["pingidm_log_retention"], "7")
        ...
        PD_BACKUP_TIME                         = try(var.products[var.product_name].parameters["pingidm_backup_time"], "02:00")
        LDIF_BACKUP_TIME                       = try(var.products[var.product_name].parameters["pingidm_ldif_backup_time"], "03:00")
  ...
  depends_on = [
                  aws_iam_policy_attachment.pingidm_server
  ]
...
//  region_backup_mins = try(var.products[var.product_name].parameters["pingidm_ldif_backup_time"], "03:00")
}
