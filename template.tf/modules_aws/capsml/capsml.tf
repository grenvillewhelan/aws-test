resource "aws_instance" "capsml_server" {
  count         = var.number_servers
   instance_type = var.instance_type
   ami           = data.aws_ami.capsml_ami.id
  ...
  user_data = templatefile("${path.module}/scripts/capsml.sh", {
      CUSTOMER                = var.customer_name
      ENVIRONMENT             = terraform.workspace
      REGION                  = var.my_manifest.region_name
      REGION_ALIAS            = var.my_manifest.region_alias
      BUILD_VERSION           = data.aws_ami.capsml_ami.name
  ...
  vpc_security_group_ids = concat([var.security_group_ids[basename(abspath(path.module))]], aws_security_group.capsml[*].id)
  ...
  depends_on = [ aws_security_group.capsml ]
}
