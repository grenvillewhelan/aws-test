resource "aws_instance" "capsmp_server" {
  ...
  ami           = data.aws_ami.capsmp_ami.id
...
EOF
  ...
  vpc_security_group_ids = concat([var.security_group_ids["${var.product_name}"]], aws_security_group.capsmp[*].id)

  tags = {
    Name        = join ("",["${var.product_name}-",count.index])
    ServerType  = "CyberarkPSMP"
...
//  value = [
//    for instance in aws_instance.capsmp_server : 
